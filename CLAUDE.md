...
### Analysis Pipeline (12 default analyzers + rug-pull when --monitor is set, run sequentially per file)
...
6. **ScriptRisk** (`internal/engine/scriptrisk/`) - Evidence scanner for local Python and shell scripts. Binds decoded, character-constructed, or remotely fetched Python values to `exec`/`eval`, identifies sensitive local context paired with a bound HTTP write, resolves structured pathlib/subprocess systemd persistence, and flags real pip/npm commands using unencrypted dependency sources. Emits `PY_DECODE_EXEC_001`, `PY_REMOTE_FETCH_EXEC_001`, `PY_CONTEXT_EXFIL_001`, `SC-EX-007` (migrated from YAML), `SHELL_UNSAFE_PIP_SOURCE_001`, and `SHELL_UNSAFE_NPM_SOURCE_001`.
...
11. **NLP Analyzer** (`internal/engine/nlp/`) - Goldmark AST walker for markdown; JSON/YAML string extractor for structured files. Keyword classification with proximity weighting. Detects prompt injection, authority claims, credential+exfil combos.
12. **ToxicFlow** (`internal/engine/toxicflow/`) - Single-file source/sink co-occurrence + cross-file capability correlation (`crossfile.go`). Detects dangerous capability combinations within and across files in the same directory. Flat-dir filter (>50 files) prevents FPs on registries.
...
- Rule count (currently 192 YAML / 255 cataloged)
