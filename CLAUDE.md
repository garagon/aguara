...
6. **ScriptRisk** (`internal/engine/scriptrisk/`) - Evidence scanner for local Python and shell scripts. Binds decoded, character-constructed, or remotely fetched Python values to `exec`/`eval`, resolves structured pathlib/subprocess systemd persistence, and flags real pip/npm commands using unencrypted dependency sources. Emits `PY_DECODE_EXEC_001`, `PY_REMOTE_FETCH_EXEC_001`, `SC-EX-007` (migrated from YAML), `SHELL_UNSAFE_PIP_SOURCE_001`, and `SHELL_UNSAFE_NPM_SOURCE_001`.
...
- Rule count (currently 192 YAML / 254 cataloged)
